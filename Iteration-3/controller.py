...
class Controller:
    def __init__(self):
        self.data = Data()
        self.vocab_size, self.embedding_size, self.pretrained_weights = self.data.load_embedding(dimension=300)
    ...
    def predict_on_test_file(self):
        ...
        if exists(self.model_file):
            ...
            for question in x:
                answer = ''
                for _ in range(0,3):
                    a = self.data.generate_next(model, question)
                    answer = answer + "'" + a + "',"
